// Định nghĩa class DressingRoom
class DressingRoom {
  constructor(data) {
    this.data = data;
  }

  // Hiển thị tabs và items
  renderTabs() {
    const $tabMenu = $(".nav-pills");
    const $tabContent = $(".tab-content");

    this.data.navPills.forEach((pill, index) => {
      const $navItem = $("<li>").addClass("nav-item");
      const $navLink = $("<a>")
        .addClass("nav-link")
        .attr("data-toggle", "pill")
        .attr("href", `#${pill.tabName}`)
        .text(pill.showName);

      $navItem.append($navLink);
      $tabMenu.append($navItem);

      const $tabPane = $("<div>")
        .addClass("tab-pane fade")
        .attr("id", pill.tabName)
        .html(
          this.data.tabPanes
            .filter((pane) => pane.type === pill.type)
            .map(
              (pane) =>
                `<div class="item" data-id="${pane.id}" data-type="${pane.type}">
                   <img src="../assets/images/${pane.imgSrc_png}" alt="${pane.name}" />
                 </div>`
            )
            .join("")
        );
  ...
  // Xử lý thử đồ khi click
  handleTryOn() {
    $("#tryOnButton").on("click", () => {
      // Lấy items được chọn
      const selectedItems = $(".nav-pills .nav-link.active").attr("href").substring(1);

      // Thử đồ trên mô hình
      this.data.navPills.forEach((pill) => {
        const selectedItem = $(`.tab-content .tab-pane#${pill.tabName} .item.active`);

        if (selectedItem.length > 0) {
          const imgSrc = selectedItem.find("img").attr("src");
          this.dressUp(pill.type, imgSrc);
        }
      });
    });
  }
  ...
  // Hàm thử đồ
  dressUp(type, imgSrc) {
   $(`.${type}`).css("background-image", `url(${imgSrc})`);
  }

  // Khởi tạo Dressing Room
  initDressingRoom() {
    this.renderTabs();
    this.handleTryOn();
  }
...
// Khởi tạo DressingRoom và chạy khi tài liệu đã sẵn sàng
$(document).ready(() => {
  const dressingRoom = new DressingRoom(data);
  dressingRoom.initDressingRoom();
});
